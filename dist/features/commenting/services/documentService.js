...
export class DocumentService {
   ...
   */ async checkIfDocumentExists(documentId) {
        return withErrorHandling(async ()=>{
            ...
            return response.ok;
        }, 'Error checking if document exists', false);
    }
    /**
   ...
   */ async detectRichTextField(collection, docId) {
        return withErrorHandling(async ()=>{
            ...
            return fieldName;
        }, 'Error fetching document to determine field name', 'content' // Default field name as fallback
        );
    }
    /**
   ...
   */ async saveDocument(editor, documentId) {
        return withErrorHandling(async ()=>{
            // Get the document ID from the URL or props
            const docId = documentId || getDocumentIdFromUrl();
            // Get the collection name from the URL path
